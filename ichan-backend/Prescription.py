...
_ORIGINAL_FILE = Path(__file__).resolve().parent / "original_sources" / "Prescription.py"
...
def run_prescription(*args, **kwargs):
    print("=== RUNNING Prescription ===")
    try:
        ns = _execute_original()
        target = ns.get("run_prescription")
        if callable(target):
